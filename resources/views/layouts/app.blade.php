            ...
            @if (Auth::check())
                <div class="profile" style="display: flex; align-items: center; gap: 15px;">
                    ...
                    <form action="{{ route('logout') }}" method="POST" style="margin: 0;">
                        @csrf
                        <button type="submit" style="background-color: #e4cc2f; color: white; border: none; padding: 8px 15px; border-radius: 20px; font-size: 0.9rem; font-weight: 500; cursor: pointer; transition: background-color 0.3s;">
                            Logout
                        </button>
                    </form>
                </div>
